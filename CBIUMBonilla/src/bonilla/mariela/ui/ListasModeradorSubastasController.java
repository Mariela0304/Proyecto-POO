...
public class ListasModeradorSubastasController implements Initializable {
    ...
    public void listasSubastas() {
        try {
            ...
            for (Subasta dato : dao.listarSubastas().values()) {
                LocalDate fecha = dato.getFechaVencimiento();
                System.out.println(fecha);
                dato.setTiempoFaltante(calcularTiempoFaltante(fecha));
    ...
    public void verInfoSubasta(ActionEvent event) throws IOException {
        ...
        Subasta subasta = tabla_subastas.getSelectionModel().getSelectedItem();
        ...
        gestorInfoSubasta.guardarDatos(subasta.getId(),subasta.getFechaVencimiento(),
                ...
                subasta.getIdentificacionVendedor(),
                subasta.getNomVendedor(),
                subasta.getPrecioMinimo(), subasta.getCantidad_items(), subasta.getEstado());



        Parent ruta = FXMLLoader.load(getClass().getResource("info_subasta_moderador.fxml"));
